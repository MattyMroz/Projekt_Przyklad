...
int main(int argc, char const *argv[])
{
    ...
    case 3:
        ...
        cout << "6. UWAGA szyfrowane i deszyfrowane sa tylko znaki umieszczone w pliku alphabet.txt, co za tym idzie, tam gdzie one nie wystepuja znaki sa przekopiowane na swoich miejscach" << endl;
...
string vigenereEncrypt(string arrayVigenere[], string text, string key)
{
    ...
    // usuwanie spacji z klucza
    key.erase(remove(key.begin(), key.end(), ' '), key.end());

    // usunie wszystkie znaki białe i inne niealfabetowe z tekstu
    for (int i = 0; i < text.length(); i++)
    {
    ...
    {
        int indexLetter = uppercaseAlphabetPL.find(text2[i]);
        int indexKey = uppercaseAlphabetPL.find(key[i % key.length()]);

        if (indexLetter >= 0 && indexLetter < array_size)
    ...
    // dodanie znakow niealfabetowych
    for (int i = 0; i < notAlfabet.size(); i++)
        vigenereEncrypt.insert(notAlfabet[i], 1, text[notAlfabet[i]]);

    cout << "Tekst po szyfrowaniu: " << vigenereEncrypt << endl;
...
string vigenereDecrypt(string arrayVigenere[], string text, string key)
{
    ...
    for (int i = 0; i < text2.length(); i++)
    {
        ...
        if (indexLetter >= 0 && indexLetter < array_size)
        {
            for (int j = 0; j < array_size; j++)
                if (arrayVigenere[indexKey][j] == text2[i])
    ...
    // dodanie znakow niealfabetowych
    for (int i = 0; i < notAlfabet.size(); i++)
        vigenereDecrypt.insert(notAlfabet[i], 1, text[notAlfabet[i]]);
